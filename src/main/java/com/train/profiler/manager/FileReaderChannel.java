...
import com.train.profiler.queue.MessageWrapper;
import com.train.profiler.queue.QueueChannel;
import com.train.profiler.util.LogUtil;
import com.train.profiler.util.SleepUtil;
...
public class FileReaderChannel implements Task {
    ...
    private void loadMessages() {
    	//如果当前没有文件可读，则等待FileWriteChannel 写新的文件
        if (readFile == null) {
            try {
    ...
    private void readNextFile() {
        ...
        //切换文件前，释放前一个文件的readchannel
        if (readFileChannel != null) {
            try {
            	unmap(readMappedByteBuffer);
    ...
    private void loadFileData() {
        ...
        while ((length = getMessageLength()) > 0 || dataFileManager.isEmpty()) {
            ...
            //length<=0,position in the  file has to be end, wait to write....
            if (length <= 0) {
                readMappedByteBuffer.position(readPos);
